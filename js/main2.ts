import { chartBaseOptions, mapDataValuesToChartSeries, calculateNiceInterval } from "./chart";
...
function initialiseECharts(shouldReplaceState: boolean = false) {
  ...
  xChart.setOption({
    title: {
      text:
        state.yLabel.toLowerCase() !== "" ? state.yLabel : "X Plot",
    },
    xAxis: {
      name: state.xLabel,
    },
  ...
  mrChart.setOption({
    title: {
      text:
        state.yLabel.toLowerCase() !== "s" ? `${state.yLabel} Range` : "MR Plot",
    },
    xAxis: {
      name: state.xLabel,
    },
...
function ceilToNice(val: number) {
  if (Math.abs(val) < 10) {
...
function floorToNice(val: number) {
  if (Math.abs(val) < 10) {
...
function adjustChartAxis(stats: _Stats) {
  ...
  // Calculate nice interval for yAxis
  const niceInterval = calculateNiceInterval(stats.xdataPerRange);
  
  // Calculate the number of divisions based on our min/max and interval
  const divisions = Math.ceil((xChartYMax - xChartYMin) / niceInterval);
  ...
  xChart.setOption({
    yAxis: {
      ...
      splitNumber: Math.min(7, divisions), // Explicitly limit to at most 7 divisions
    },
    xAxis: { min: xMin, max: xMax },
  });
  
  // Do the same for MR chart
  const mrInterval = calculateNiceInterval([stats.movementsPerRange.flat()]);
  const mrMax = ceilToNice((1 + PADDING_FROM_EXTREMES) * stats.mrchartMax);
  ...
  mrChart.setOption({
    yAxis: {
      ...
      max: mrMax,
      interval: mrInterval,
      splitNumber: Math.min(7, Math.ceil(mrMax / mrInterval)),
    },
    xAxis: { min: xMin, max: xMax },
  });
}
